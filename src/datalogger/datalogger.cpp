...
void DataLogger::initMonitorToolView()
{
	...
	for (libm2k::analog::DMM* dmm : m_dmmList) {
		...
		menu->init(QString::fromStdString(dmm->getName()),new QColor("green"));

		ChannelWidget *mainCh_widget =
				m_toolView->buildNewChannel(m_monitorChannelManager, menu, true, chId, false, false, QColor("green"),
							    QString::fromStdString(dmm->getName()), QString::fromStdString(dmm->getName()));
		...
		for (auto channel : dmm->readAll()) {
			...
			channelMenu->init(QString::fromStdString(dmm->getName() + ": " + channel.id),new QColor(channelColor));

			ChannelWidget *ch_widget =
					m_toolView->buildNewChannel(m_monitorChannelManager, channelMenu , true, chId, false, false,
			...
			monitor->init(0,QString::fromStdString(channel.unit_name),QString::fromStdString(channel.unit_symbol),
				      QString::fromStdString(dmm->getName() + ": " + channel.id),channelColor );
...
void DataLogger::updateChannelWidget(int ch)
{
	...
	if (m_activeChannels.contains(ch)) {
		...
		m_activeChannels[ch].second->updateValue(updatedRead.value,QString::fromStdString(updatedRead.unit_name),
